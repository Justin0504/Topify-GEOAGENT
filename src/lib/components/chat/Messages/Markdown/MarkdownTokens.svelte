...
{#each tokens as token, tokenIdx (tokenIdx)}
	...
	{:else if token.type === 'details'}
		{@const isToolCall = token?.attributes?.type === 'tool_calls' || (token?.attributes?.name && token?.attributes?.done === 'true')}
		{@const forceCollapsed = isToolCall ? true : false}
		<Collapsible
			title={token.summary}
			open={forceCollapsed ? false : ($settings?.expandDetails ?? false)}
